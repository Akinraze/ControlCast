...
VERSION = "InputCast_Windows_v4_0"
...
# Windows modifier-only polling state. OBS hotkey bindings require a primary key,
# so modifier-only shortcuts (Shift, Ctrl, Alt, Win, or combinations) are
# detected directly with GetAsyncKeyState and then fed through the same
...
def apply_ui_binding(index):
    ...
    # Modifier-only input is polled directly on Windows instead.
...
def show_native_message(title, message, error=False, warning=False):
    """Detached Windows popup so OBS scripting never blocks."""
    try:
        icon = "Information"
        if error:
            icon = "Error"
        ...
        ps_script = ps_script.replace("__TITLE__", title_b64).replace("__MESSAGE__", message_b64).replace("__ICON__", icon)
        ...
        log("Could not show diagnostic window: %s" % exc)
        return False
...
def script_description():
    return (
        "<b>%s</b><br><br>"
        "Windows test build for <b>InputCast</b> named profiles and live switching.<br>"
        ...
        "<b>Shortcut assignment:</b> choose a Key and Modifiers directly here, choose 'Modifier only' for Shift/Ctrl/Alt/Win without a second key, or leave Key on 'Assign / keep binding in OBS Hotkeys' for controllers/special hardware.<br><br>"
