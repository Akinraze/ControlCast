...
# Windows left/right indicators, twenty programmable extra inputs, named profiles, and bounded mouse movement.
...
VERSION = "MouseCast_Windows_v4_0"
...
def script_description():
    return (
        "<b>%s</b><br><br>"
        "Windows test build for <b>MouseCast</b> with named mouse profiles and live profile switching.<br>"
        ...
        "Startup self-check logs PASS silently and opens a detached Windows dialog only for warnings/errors.<br>"
...
def modifier_combo_is_down(modifier_code):
    ...
    checks = []
    if "SHIFT" in mods:
        checks.append(is_vk_down(VK_SHIFT))
    if "CTRL" in mods:
        checks.append(is_vk_down(VK_CONTROL))
    if "ALT" in mods:
        checks.append(is_vk_down(VK_MENU))
    if "COMMAND" in mods:
        checks.append(is_vk_down(VK_LWIN) or is_vk_down(VK_RWIN))
...
def collect_self_check():
    ...
    if user32 is None:
        errors.append("Windows USER32 input API unavailable")
    else:
        passes.append("Windows mouse input API")
...
def show_native_message(title, message, error=False, warning=False):
    """
    Launch the result dialog in a separate PowerShell process.

    IMPORTANT:
    Do NOT call MessageBoxW directly from the OBS Python callback thread.
    A modal Win32 message box blocks OBS scripting/timers until it is closed.
    The detached process keeps the dialog independent of OBS.
    """
    try:
        icon = "Information"
        if error:
        ...
        log("Could not open non-blocking diagnostic window: %s" % exc)
        return False
...
def add_modifier_list(props, key, label):
    ...
    choices = [
        ...
        ("Win / Command", "COMMAND"),
        ("Win/Cmd + Ctrl", "COMMAND+CTRL"),
        ("Win/Cmd + Shift", "COMMAND+SHIFT"),
        ("Win/Cmd + Alt", "COMMAND+ALT"),
    ]
    for display, value in choices:
        obs.obs_property_list_add_string(prop, display, value)
    return prop
...
def poll_movement():
    ...
    dx = point.x - last_cursor[0]
    dy = point.y - last_cursor[1]
...
def poll_mouse_buttons():
    ...
    try:
        ...
        new_left = is_button_down(VK_LBUTTON)
        new_right = is_button_down(VK_RBUTTON)
...
def script_load(settings):
    ...
    if user32 is None:
        log("ERROR: Windows user32 API is unavailable.")
